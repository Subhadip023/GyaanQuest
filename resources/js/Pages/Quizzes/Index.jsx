...
import defualtQuizData from '@/utils/defualtQuizData';
...
function Index({ quizzes }) {
  ...
  const [filterQuizzes, setFilterQuizzes] = useState(quizzes);
  ...
  const openEditQuizmodal = (quiz) => {
    editQuizForm.setData(quiz);
    setOpenEditQuizform(true);
  }

  useEffect(() => {
    setFilterQuizzes(quizzes)
  }, [quizzes])
  ...
  const filterSearchQuizzes = (value) => {
    ...
    setFilterQuizzes(filtered);
  };



  return (
    <AdminLayout title='Quiz' heading='Quiz' searchFunction={filterSearchQuizzes} serachBoxPlaceHolder="Search by Quiz name">
      ...
      <section >
        <div className='w-full flex items-center justify-end gap-x-2'>
          <Button onClick={(e) => { e.preventDefault(); setAddQuizModal(true) }}>Add Quiz</Button>
        </div>
        ...
        <Table columns={['#', 'Name', 'Description', 'questions', 'Visible', 'Active', 'Action']}>
          {
            filterQuizzes.length === 0 ? (
              <TableRow>
                <TableData colSpan="7" className="text-red-500 font-bold text-center py-5">
                  No Quiz found
                </TableData>
              </TableRow>
            ) :
              filterQuizzes.map((quiz, index) => (<TableRow key={index || quiz.id} isLast={index === filterQuizzes.length - 1}>
                <TableData>{index + 1}</TableData>
                <TableData>{quiz.name}</TableData>
                <TableData>{quiz.description ? quiz.description : "No Description available"}</TableData>
                <TableData>{quiz.question.length}</TableData>
                <TableData>{quiz.display}</TableData>
                <TableData>{quiz.active ? 'Active' : "Not Active"}</TableData>
                <TableData>
                  <div className='flex w-full items-center gap-x-2'>
                    <EditBtn onClick={() => openEditQuizmodal(quiz)} />
                    <DeleteBtn onClick={() => {
                      setOpenAlert(true);
